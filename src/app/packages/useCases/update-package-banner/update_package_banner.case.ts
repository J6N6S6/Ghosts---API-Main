...
@Injectable()
export class UpdatePackageBannerCase {
  constructor(
  ...
  async execute(
    ...
    try {
      ...
      if (pack.banner)
        try {
          await this.uploadService.deleteFile(pack.banner);
        } catch (err) {
          console.log(err);
        }
      ...
      const uploadResult = await this.uploadService.uploadFile({
        buffer: banner,
        filename: pack.title,
        location: ['packages', 'banner'],
      });
      pack.banner = uploadResult.url;

      await this.packagesRepository.updatePackBanner(pack);
