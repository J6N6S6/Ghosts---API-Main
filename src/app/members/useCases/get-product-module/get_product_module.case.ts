...
import { GetProductModuleDTO } from './get_product_module.dto';
import { ProductModules } from '../../types/product_modules.type';

@Injectable()
export class GetProductModuleCase {
  constructor(
  ...
  async execute({
    ...
    module_id,
    lessons = false,
  }: GetProductModuleDTO): Promise<ProductModules> {
    ...
    const productModule = await this.productsModulesRepository.findBy({
      where: {
        id: module_id,
    ...
    if (!productModule) throw new ClientException('Módulo não encontrado');
